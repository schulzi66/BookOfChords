  ...
  implements OnInit
{
  ...
  ngOnInit() {
    if (this._songService.selectedBandForOverview) {
      ...
      this._subscriptions$.add(
        this._songService
          .getSongsForUserByBandId(
            this._authService.user.uid,
            this._selectedBand.id
          )
          .pipe(take(1))
    ...
    this._subscriptions$.add(
      this._configurationService.configuration$.subscribe(
        (configuration: Configuration) => {
          this._songService.filterArchivedSongs =
            configuration.filterArchivedSongsInitially;
  ...
  private storeFilterActiveConfig(filterActive: boolean): void {
    ...
    this._configurationService.configuration.filterArchivedSongsInitially =
      filterActive;

    this._configurationService.saveConfigurationForUser(
  ...
  public searchForSong(searchString: string): void {
    this.filteredSongs = this._songs.filter((song) =>
  ...
  public chooseRandomSong(): void {
    if (this._songs.length > 0) {
      this._router.navigate([
        "/songs/details",
        this._songs[Math.floor(Math.random() * this._songs.length)].id,
      ]);
    }
  }
  ...
  public selectBand(): void {
    const bottomSheetRef: MatBottomSheetRef =
      this._bottomSheetService.showBandSelection({
